...
// Function to read package names and paths from the local packages database
std::vector<std::pair<std::string, std::string>> readPackagesDatabase(const std::string& filename) {
    std::ifstream packagesFile(filename);
    if (!packagesFile) {
    ...
    while (std::getline(packagesFile, line)) {
        std::istringstream iss(line);
        std::string name, path;
        if (iss >> name >> path) {
            packages.emplace_back(name, path); // Store package name and path
        }
    }
...
void installPackage(const std::string& packageName) {
    auto packages = readPackagesDatabase("packages.txt");

    for (const auto& [name, path] : packages) {
        if (name == packageName) {
            // Create a new directory for the package with a proper path separator
            std::string packageDir = installedPackagesDir + packageName + "/";
            fs::create_directories(packageDir);

            // Copy package files to the new directory
            fs::copy(path, packageDir, fs::copy_options::recursive | fs::copy_options::overwrite_existing);
            std::cout << "Installed package: " << packageName << " from local database." << std::endl;
            return;
        }
    }

    std::cout << "Package '" << packageName << "' not found in local database." << std::endl;
}

void installPackageFromGitHub(const std::string& packageName, const std::string& gitLink) {
    std::cout << "Attempting to clone package from provided GitHub link..." << std::endl;

    // Ensure the package name is sanitized to avoid issues in directory creation
    std::string sanitizedPackageName = packageName; // You may want to add more sanitization if needed
    std::string command = "git clone " + gitLink + " " + installedPackagesDir + sanitizedPackageName;
...
// Function declarations for Tetra
void tetraInstall(const std::string& packageName);
void tetraInstallFromGitHub(const std::string& packageName, const std::string& gitLink);
void tetraUninstall(const std::string& packageName);
void handleTetraCommand(const std::vector<std::string>& tokens);
std::vector<std::string> split(const std::string& str, char delimiter); // This function seems important

// Tetra installation functions
void tetraInstall(const std::string& packageName) {
    installPackage(packageName);  // Install from local database (if needed)
}

void tetraInstallFromGitHub(const std::string& packageName, const std::string& gitLink) {
    installPackageFromGitHub(packageName, gitLink);  // Install from GitHub
}

void tetraUninstall(const std::string& packageName) {
    uninstallPackage(packageName);  // Uninstall package
}

// Handle tetra commands
void handleTetraCommand(const std::vector<std::string>& tokens) {
    ...
    const std::string& command = tokens[1]; // "install"
    const std::string& packageName = tokens[2]; // package name
    const std::string& gitLink = tokens[3]; // GitHub link

    if (command == "install") {
        // Validate if the GitHub link is provided correctly
        if (gitLink.find("https://github.com/") == 0) {
            tetraInstallFromGitHub(packageName, gitLink); // Install from GitHub
        } else {
            std::cout << "Error: Invalid GitHub link." << std::endl;
        }
    } else if (command == "uninstall") {
        tetraUninstall(packageName);
    } else {
        std::cout << "Error: Unknown command. Usage: tetra install <package_name> <git_link> or tetra uninstall <package_name>" << std::endl;
    }
}

// List available packages in the database
void tetraList() {
    auto packages = readPackagesDatabase("packages.txt");
    if (packages.empty()) {
        std::cerr << "Error: Package database not found or empty!" << std::endl;
        return;
    }
    ...
    if (tokens[1] == "install") {
        if (tokens.size() != 4) { // Check for exactly 4 tokens
            ...
            if (tokens[3].find("https://github.com/") == 0) {
                ...
                tetraInstallFromGitHub(tokens[2], tokens[3]); // No ambiguity here
            } else {
                std::cout << "Error: Invalid GitHub link." << std::endl;
            }
        }
    } else if (tokens[1] == "uninstall" && tokens.size() == 3) {
        tetraUninstall(tokens[2]);
